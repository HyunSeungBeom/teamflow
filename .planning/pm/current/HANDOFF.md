...
## 3. DB 스키마 (Sprint 2 추가)

```sql
CREATE TABLE issues (
    no              BIGSERIAL PRIMARY KEY,
    project_no      BIGINT NOT NULL REFERENCES projects(no) ON DELETE CASCADE,
    ...
    assignee_no     BIGINT REFERENCES users(no),
    position        INT NOT NULL DEFAULT 0,          -- 컬럼 내 순서
    due_date        DATE,
    create_date     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    update_date     TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    UNIQUE(project_no, issue_key)
);

CREATE INDEX idx_issue_project_status ON issues(project_no, status);
CREATE INDEX idx_issue_assignee ON issues(assignee_no);
```
