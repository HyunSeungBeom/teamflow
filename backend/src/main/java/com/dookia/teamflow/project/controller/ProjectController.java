...
public class ProjectController {
    ...
    public ResponseEntity<ApiResponse<ProjectDto.Response>> create(
        ...
        ProjectDto.Response created = projectService.create(workspaceNo, userNo, request);
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(created));
    }

    @Operation(summary = "워크스페이스 하위 프로젝트 목록")
    @GetMapping("/api/workspaces/{workspaceNo}/projects")
    public ResponseEntity<ApiResponse<List<ProjectDto.SummaryResponse>>> list(
        @AuthenticationPrincipal Long userNo,
        @PathVariable Long workspaceNo
    ) {
        return ResponseEntity.ok(ApiResponse.success(projectService.listInWorkspace(workspaceNo, userNo)));
    }

    @Operation(summary = "프로젝트 상세")
    @GetMapping("/api/projects/{projectNo}")
    public ResponseEntity<ApiResponse<ProjectDto.Response>> getDetail(
        @AuthenticationPrincipal Long userNo,
        @PathVariable Long projectNo
    ) {
        return ResponseEntity.ok(ApiResponse.success(projectService.getDetail(projectNo, userNo)));
    }
}
